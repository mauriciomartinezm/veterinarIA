...
export const loginUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      "SELECT * FROM usuario WHERE Cedula = ? AND Contrasena = ?",
...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO usuario(Cedula, Contrasena, PrimNombre, PrimApellido, IDCargo) VALUES (?, ?, ?, ?, ?)",
      [Cedula, Contrasena, PrimNombre, PrimApellido, 4]
    );
